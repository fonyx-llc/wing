...
use crate::wing::prepare::prepare_for_build;
...
pub fn handle() {
	println!("\n| ---   ---   ---   ---   ---   ---   ---   ---   ---   ---   ---   --- |");
	println!("|                                                                       |");
	println!("| Fonyx Wing vBranch. (c) 2020 Fonyx Software. All rights reserved.     |");
	println!("| Welcome to Wing Server, a mimic of the Fonyx Unity build system.      |");
	println!("|                                                                       |");
	...
	let (job_raw, file_path) = if command_line_args.len() > 2 {
		...
		config_load_spinner.stop_with_message("Insufficient arguments provided, Usage: wing <job> <root-directory>".into());
		exit(1);
	};
	...
	let job: Job = match job_raw.as_str() {
		...
		_ => {
			config_load_spinner.stop_with_message(format!("Invalid job (build-to-target | run-target | run-test | emulate-target | emulate-test | build-and-run-target | build-and-test) '{}', Usage: wing <job> <root-directory>", job_raw));
			exit(1);
		}
	};
